...
This switch was designed to detect the miRNA {target} at a temperature of {celsius}°C.
We tested it against every mature <i>{species}</i> miRNA in miRBase and our analysis shows that at this temperature it is best used to detect {inferred_target_name}.
...
[[File:{part}_graph.png|500px|center]]
...
"""
BAD_SWITCH = """As per the above, we cannot confirm that this switch accurately detects the desired miRNA sequence."""
UNRELIABLE_SWITCH = """
The miRNA most targeted by this switch heavily fluctuates based on temperature.
...
class Contribution:
    ...
    def save(
        ...
        crt_res.savefig(
            os.path.join(partdir, toehold_name + "_graph.png"), z_score=1, dpi=300
        )
        ...
        activation = to_same_dp(thtest_res.target_activation * 100, activation_se)
        desc = (
            TEXT.format(
                part=toehold_name,
                target=mirna,
